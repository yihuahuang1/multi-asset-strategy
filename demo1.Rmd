...
server22 <- function(input, output) {
 ...
 # Add a trend line by connecting volumes with a line
  v <- add_trace(v, x = ~date, y = ~volume, type = 'scatter', mode = 'lines',
                 line = list(color = 'green'), name = 'Trend Line 交易量趋势线',
                 showlegend = FALSE)
  ...
  output$instructions <- renderUI({
    HTML('
      ...
      <ul>
        ...
        <h4>交易量图表 Volume Chart:</h4>
         <li>1. 框选时间轴放大,移动<strong>时间轴</strong>缩放,<strong>双击恢复默认</strong>时间轴。 Select on the intended area to zoom in, move the <strong>time line tool</strong> to zoom in/out, and <strong>double click to restore the default.</strong>set time line. </li> 
        <li>2. <strong>移动光标到成交量图表</strong>可见该交易(日)周期的交易量和趋势数据。<strong>Mover cursor or hoover on the volume chart</strong> to observe the exact trading volume data and trend for the (day) duration.</li>
      </ul>
    ')
